  ...
  return new Promise(function(accept, reject) {
    ...
    request(options, function(error, r) {
      if (error) {
        ...
        return reject(new Error("Moxie Box at URL " + url + " doesn't exist. If you believe this is an error, please contact Moxie support."));
...
function readBoxConfig(destination) {
  var possibleConfigs = [
    path.join(destination, "moxie-box.json"),
    path.join(destination, "moxie-init.json")
  ];
...
function cleanupUnpack(boxConfig, destination) {
  ...
  needingRemoval.push("moxie-box.json");
  needingRemoval.push("moxie-init.json");
